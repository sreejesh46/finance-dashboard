...
const Register = () => {
  ...
  const { register, showAuthTransition, hideAuthTransition } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    if (success) {
      showAuthTransition('Setting up your FinDash workspace...');
      window.setTimeout(() => {
        navigate('/');
        window.setTimeout(() => {
          hideAuthTransition();
  ...
  return (
    ...
    >
      <form className="space-y-3.5" onSubmit={handleSubmit}>
        ...
        >
          {loadingLocal ? <Loader2 className="w-6 h-6 animate-spin" /> : 'Sign Up'}
        </button>
      </form>
    </AuthPanel>
  );
